...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    const s = localStorage.getItem('store'); if (s) {
      setStore(JSON.parse(s)); setName(JSON.parse(s).name); setPhone(JSON.parse(s).phone || '');
    }
  }, []);
  const save = async () => {
  ...
  useEffect(() => {
    if (store) api.get(`/qrcode.php?slug=${store.slug}`).then(r => setQrUrl(r.data.url));
  }, [store]);
